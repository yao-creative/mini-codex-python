...
class TurnLoopTypeState(TypeState):
    pass


@dataclass(frozen=True)
class Idle(TurnLoopTypeState):
    pass


@dataclass(frozen=True)
class Running(TurnLoopTypeState):
    request_id: str  # handle to the in-flight LLM call — prevents double-dispatch (typestate point)


@dataclass(frozen=True)
class AwaitingTool(TurnLoopTypeState):
...
@dataclass(frozen=True)
class Error(TurnLoopTypeState):
    reason: str
    recoverable: bool


TurnLoopState = Idle | Running | AwaitingTool | Error
